...
class WanFineTuneSetup(
    ...
    def create_parameters(
        ...
        if model.transformer_2 is not None:
            self._create_model_part_parameters(parameter_group_collection, "transformer_2", model.transformer_2, config.transformer_2, freeze=ModuleFilter.create(config), debug=config.debug_mode)
    ...
    def __setup_requires_grad(
        ...
        if model.transformer_2 is not None:
            self._setup_model_part_requires_grad("transformer_2", model.transformer_2, config.transformer_2, model.train_progress)
...
factory.register(BaseModelSetup, WanFineTuneSetup, ModelType.WAN_I2V_A14B, TrainingMethod.FINE_TUNE)
